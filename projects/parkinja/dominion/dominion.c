...
//currentPlayer, choice1, choice2, handPos
int Mine_Efct(struct gameState *state, int choice1, int choice2, int handPos) {  
  //Get the current player  
  int currentPlayer = whoseTurn(state);
  int j;
  int i;
  ...
  //Original Code------------------------
  //BUG: 99 supplied to hand rather than currentPlayer
  //Effect: Breaks game, segmentation fault
  j = state->hand[99][choice1];  //store card we will trash
...
//Needs state, handPos
int Treasure_Efct(struct gameState *state, int handPos) {
  //Get the current player  
  int currentPlayer = whoseTurn(state);
  int i;
