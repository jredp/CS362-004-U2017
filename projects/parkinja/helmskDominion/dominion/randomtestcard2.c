...
int main() {
    ...
    for (testsRan=0; testsRan<TEST_NUMBER; testsRan++) {
      ...
      if (G.handCount[player] == 0) G.handCount[player] = 1;
      ...
      /*-------------------*/
      //printf("Effect\n");      //DEBUG            
      assertResult = cardEffect(village, 1, 1, 1, &G, handPos, 0);
      tCe += assertResult;
      totTests += 1;
      ...
      handBool = custAssert((beforeCheckHandCount == afterCheckHandCount), "Card Effect: HandCount Net Zero Gain");
      tHc += handBool;
      totTests += 1;
      //------DEBUG-----
      // printf("---handBool: %d\n", handBool); //DEBUG
      ...
      totTests += 1;
      // state->numActions = 1;
      sActions += custAssert(G.numActions == 3, "Game State: Actions + 2 (3 total)");
      totTests += 1;
      // state->numBuys = 1;
